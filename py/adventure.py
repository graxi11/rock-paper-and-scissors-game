...
if answer == 'left':
    ...
    if answer == 'back':
        ...
        if answer == 'yes':
            #yes
            answer = input('you talk to the stranger and they gave you a map to the GOLD mine (move/rest)? ')

            if answer == 'move':
                #move
                answer = input(
                    'good job!, you are on the part to the tresure and you found 3 fler fire, there is a dark tunnel ahead and a field of very tall grass, which will choses (tunnel/field)? ')
                if answer == 'tunnel':
                    print('now you are able to pass the dark tunnel with the use of the fler fire, and that was a DEAD END! ')
                    #WAITNG ON THIS......
                elif answer == 'field':
                ...
                #rest
                answer = input('how many min do you want to rest for (5min/10min)? ')

                if answer == '10min':
                    print('you rested for too long and you got eaten by a wild beast. ')
                elif answer == '5min':
                    answer = input('you escaped the wild beast and found a small village to find food (find food/continue)? ')
                    if answer == 'find food':
                        answer = input('you found food from a stranger, do you want to eat it or not (accept)? ')
                        if answer == 'accept':
                            print('the food was poisoned, and you LOSE! ')
                    elif answer == 'continue':
                        answer = input('you continued your jonney and you are on the part to find the GOLD MINE, and on way you found a wild cat do you (run/fight)? ')
                        if answer == 'run':
                            print('you were out runed by the wild cat, and you LOSE')
                        elif answer == 'fight':
                            answer = input('you were able to kill the cat cause you had the skill, GOOD JOB!, after a long walk you found a dark cave do you want to (enter/leave)? ')
                            if answer == 'leave':
